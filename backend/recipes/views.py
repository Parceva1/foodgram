...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().prefetch_related('tags')  # Добавлено prefetch_related('tags')
...
class ShoppingCartView(APIView):
    ...
    def post(self, request, pk):
        ...
        if ShoppingCart.objects.filter(user=request.user, recipe=recipe).exists():
            return Response({'error': 'Рецепт уже добавлен в корзину.'}, status=status.HTTP_400_BAD_REQUEST)

        serializer = ShoppingCartSerializer(data={'recipe': recipe.id}, context={'request': request})
        if serializer.is_valid():
            serializer.save(user=request.user)
            return Response({'id': recipe.id, 'name': recipe.name, 'image': request.build_absolute_uri(recipe.image.url), 'cooking_time': recipe.cooking_time}, status=status.HTTP_201_CREATED)
...
class DownloadShoppingCartView(APIView):
    ...
    def get(self, request):
        ...
        if not cart_items.exists():
            return Response({'error': 'Список покупок пуст.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for name, details in ingredients.items():
            shopping_list += f"{name} - {details['amount']} {details['unit']}\n"
